...
# Finish this processor, so we can prepped 2D & 3D data to 3D Slicer

class SendVoxelToIGTL(FlowFileTransform):
    class Java:
        implements = ['org.apache.nifi.python.processor.FlowFileTransform']
    class ProcessorDetails:
        version = '0.0.1-SNAPSHOT'
        dependencies = ['itk==5.3.0', 'pandas==1.3.5', 'igtl', 'tqdm==4.66.1']
        description = 'Gets a sample index for a NIfTI filepath for a particular stage in the AI stroke diagnosis pipeline located in the pandas csv dataframe column in the flow file, loads that NIfTI file as an ITK voxel, slices that voxel into a 2D image slice or keeps 3D voxel and sends data via OpenIGTLink to 3D Slicer'
        tags = ['sjsu_ms_ai', 'csv', 'itk', 'openigtlink', '3d_slicer', 'nifti']
